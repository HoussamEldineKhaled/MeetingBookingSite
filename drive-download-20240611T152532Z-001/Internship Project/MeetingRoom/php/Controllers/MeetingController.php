...
class MeetingController {
    ...
    public function getMeetingByOtherManagers() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            
            $id = $_POST['Id'];
            $meeting = $this->meetingService->getMeetingByOtherManagers($id);
            header('Content-Type: application/json');
            echo json_encode($meeting);
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    else if (isset($_POST['action']) && $_POST['action'] === 'getByOtherManagers') {
        $meetingController->getMeetingByOtherManagers();
    }
     else {
