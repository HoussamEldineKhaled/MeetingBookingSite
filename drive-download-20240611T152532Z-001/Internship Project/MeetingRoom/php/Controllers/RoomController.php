...
public function getRoomByCompany(){
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $id = $_POST['Id'];
        $meeting = $this->roomService->getRoomByCompany($id);
        header('Content-Type: application/json');
        echo json_encode($meeting);
}
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    else if (isset($_POST['action']) && $_POST['action'] === 'getByCompany') { 
        $controller->getRoomByCompany();
    } 
        else {
